...
namespace Chess
{
	...
	namespace MoveGen
	{
		...
		inline size_t generate_captures(
			const Postion& pos, int32* captures)
		{
			const player_t to_move = pos.get_turn();
			...
			uint64 caps =
				shift_pawns<7>(pawns) & pos.get_occupied(flip(to_move));

			while (caps)
			{
				const int to   = msb64(caps);
				const int from = tables.minus_7[to_move][to];
			 ...
			 */
			caps =
				shift_pawns<9>(pawns) & pos.get_occupied(flip(to_move));

			while (caps)
			{
				const int to   = msb64(caps);
				const int from = tables.minus_9[to_move][to];
			...
			if (ep_data.target != square_t::BAD_SQUARE)
			{
				for (int i = 0; i < 2; i++)
				{
					...
					else if (!(tables.set_mask[from] & pinned))
					{
						...
						const int vic = tables.minus_8[to_move][to];

						const uint64 rank_attacks =
							pos.attacks_from<piece_t::rook>(vic)
								& tables.ranks64[from];
			...
			while (promotions)
			{
				const int to   = msb64(promotions);
				const int from = tables.minus_8[to_move][to];
		...
		inline size_t generate_non_captures(
			const Postion& pos, int32* captures)
		{
			/*
