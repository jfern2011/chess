...
public:
	...
	R forward(const std::string& cmd, T... args)
	{
		...
		//decltype(sig->raise(args...)) val = sig->raise(args...);

		return sig->raise(args...);
	}
/*
	template <typename... T>
	void forward(const std::string& cmd, T... args)
	{
		std::string _cmd = preprocess(cmd);
		int id = -1;
		...
		signal_t<void,T...>* sig =
				dynamic_cast<signal_t<void,T...>*>(_id2sig[id]);
...
*/
	...
	int install(const std::string& cmd, R(*func)(T...))
	{
		...
		_id2sig[id] = new fcn_ptr<R,T...>(func);

		fcn_ptr<R,T...>* sig =
			dynamic_cast<fcn_ptr<R,T...>*>(_id2sig[id]);

		AbortIfNot(sig->is_connected(), -1);
		return id;
	}
...
protected:
	 ...
	 */
	std::vector<virtual_signal*> _id2sig;
};
