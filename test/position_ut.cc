...
void CheckMove(chess::Position* pos, std::int32_t move) {
    ...
    const bool castled_short =
            moved == chess::Piece::KING && (from - 2 == to);
    const bool castled_long  =
            moved == chess::Piece::KING && (from + 2 == to);
    ...
    if (captured != chess::Piece::EMPTY) {
        ...
        if (!enPassant) {
            ...
            EXPECT_EQ(pos->PieceOn(chess::data_tables::kMinus8<who>[to]),
                                   chess::Piece::EMPTY);
        }
    }
    ...
    if (moved == chess::Piece::ROOK) {
        ...
        if (castled) {
            ...
            if (castled_long) {
                rook_from = to + 2; rook_to = to - 1;
            } else {
                rook_from = to - 1; rook_to = to + 1;
            }
...
TEST(Position, MakeUndoMove) {
    ...
    // Pawn captures rook and promotes, takes away castling

    for (auto promoted : promotions) {
        fen = "r3k2r/1P4P1/8/1pPp4/8/1n1n4/2P5/6K1 w kq - 0 1";
    ...
    fen = "r3k2r/8/8/1PpP4/8/1n1n4/2P5/6K1 w kq c6 0 1";

    ASSERT_EQ(pos.Reset(fen), chess::Position::FenError::kSuccess);

    const std::int32_t capture_ep_left =
    ...
    const std::int32_t capture_ep_right =
        chess::util::PackMove(chess::Piece::PAWN,
                              chess::Square::B5,
                              chess::Piece::PAWN,
                              chess::Piece::PAWN,
    ...
    CheckMove<chess::Player::kWhite>(
        &pos, capture_ep_right);

    // Pawn advances by 1 square

    CheckMove<chess::Player::kWhite>(
        &pos, chess::util::PackMove(chess::Piece::EMPTY,
                                    chess::Square::C2,
                                    chess::Piece::PAWN,
                                    chess::Piece::PAWN,
                                    chess::Square::C3));

    // Pawn advnaces by 2 squares
    ...
    fen = "r3k2r/6K1/1N3n2/3B4/R1p4Q/8/8/8 w kq - 0 1";
    ...
    CheckMove<chess::Player::kWhite>(
        &pos, chess::util::PackMove(chess::Piece::ROOK,
                                    chess::Square::B6,
                                    chess::Piece::KNIGHT,
                                    chess::Piece::PAWN,
                                    chess::Square::A8));
    ...
    for (char captured : captures) {
        std::string mod_fen = fen;
        std::replace(mod_fen.begin(), mod_fen.end(), 'p', captured);

        ASSERT_EQ(pos.Reset(mod_fen), chess::Position::FenError::kSuccess);
        ...
        CheckMove<chess::Player::kWhite>(&pos, capture);
    }
    ...
    ASSERT_EQ(pos.Reset("r3k2r/8/1N6/3B4/2p5/8/8/R1n1Kn1R w KQkq - 0 1"),
                        chess::Position::FenError::kSuccess);
    ...
    CheckMove<chess::Player::kWhite>(
        &pos, chess::util::PackMove(chess::Piece::KNIGHT,
                                    chess::Square::A1,
                                    chess::Piece::ROOK,
                                    chess::Piece::PAWN,
                                    chess::Square::C1));
    ...
    for (char captured : captures) {
        std::string mod_fen = fen;
        std::replace(mod_fen.begin(), mod_fen.end(), 'n', captured);
    ...
    ASSERT_EQ(pos.Reset("r3k2r/8/1N6/3B4/2p5/8/8/R3K2R w KQkq - 0 1"),
                        chess::Position::FenError::kSuccess);
