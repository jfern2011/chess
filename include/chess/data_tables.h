...
constexpr std::uint64_t AttacksFromDiag(int square,
    ...
    int blocker =
        util::GetLsb<std::uint64_t>(occupied & NorthEastMask(square));

    if (blocker != -1)
        attacks ^= NorthEastMask(blocker);

    blocker =
        util::GetMsb<std::uint64_t>(occupied & SouthEastMask(square));

    if (blocker != -1)
        attacks ^= SouthEastMask(blocker);

    blocker =
        util::GetLsb<std::uint64_t>(occupied & NorthWestMask(square));

    if (blocker != -1)
        attacks ^= NorthWestMask(blocker);

    blocker =
        util::GetMsb<std::uint64_t>(occupied & SouthWestMask(square));
...
constexpr std::uint64_t AttacksFromRook(int square,
    ...
    int blocker = util::GetLsb<std::uint64_t>(occupied & NorthMask(square));

    if (blocker != -1)
        attacks ^= NorthMask(blocker);

    blocker = util::GetLsb<std::uint64_t>(occupied & WestMask(square));

    if (blocker != -1)
        attacks ^= WestMask(blocker);

    blocker = util::GetMsb<std::uint64_t>(occupied & EastMask(square));

    if (blocker != -1)
        attacks ^= EastMask(blocker);

    blocker = util::GetMsb<std::uint64_t>(occupied & SouthMask(square));
...
constexpr OccupancySet<512> GenDiagOccupancies(int square) {
    ...
    int set_bits[64] = {};
    const std::size_t nbits = util::GetSetBits(diags, set_bits);
...
/**
 * Get the file that a particular square belongs to as a 64-bit mask
 *
 * @param square Get the file this square lies on
 *
 * @return A bitmask representing the file \a square is on
 */
constexpr std::uint64_t InitFiles64(int square) {
    return kFileH << util::GetFile(square);
}
...
/**
 * Get the rank that a particular square belongs to as a 64-bit mask
 *
 * @param square Get the rank this square lies on
 *
 * @return A bitmask representing the rank \a square is on
 */
constexpr std::uint64_t InitRanks64( int square ) {
    return kRank1 << (8 * util::GetRank(square));
}
...
constexpr std::uint64_t InitRayExtend(int square1, int square2) {
    ...
    if (util::GetRank(square1) == util::GetRank(square2))
        return InitRanks64(square1);

    if (GetDiagH1A8(square1) == GetDiagH1A8(square2))
        return GetDiagH1A8(square1);

    if (util::GetFile(square1) == util::GetFile(square2))
        return InitFiles64(square1);
 ...
 *
 * @note We initialize this at runtime because it takes forever to compile as
 *       constexpr
 */
 ...
 */
constexpr auto kFiles64 = internal::CreateTable<64>(internal::InitFiles64);
 ...
 */
constexpr auto kRanks64 = internal::CreateTable<64>(internal::InitRanks64);
